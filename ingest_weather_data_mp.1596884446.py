...
def get_loaded():
    with psycopg2.connect(
        ...
        conn.commit()
        return set(row["url"] for row in cursor)


def load_one_url(iurl):
    with psycopg2.connect(
        user="weather",
        password="xxx",
        database="weather",
...
def main():
    ...
    for ires in worker_pool.imap_unordered(load_one_url, generate_urls()):
        print(ires)
